...
export interface MarketingRule {
  ...
  /** 4-6 FAQ entries, rendered as visible on-page Q&A. */
...
,
  ...
  title: "FAQ Coverage: Question Content An Answer Engine Cannot Lift",
  metaDescription:
    "Question-phrased H2s whose answers no machine can lift cleanly cost you AI citations. How aeo/faq-coverage finds them, and why the fix is not markup.",
  primaryKeyword: "FAQ coverage",
  oneLiner:
  ...
  howToFix: [
    "Give every question its own heading and put the complete answer in the first paragraph directly beneath it, so a machine can lift one pair without reassembling it from elsewhere on the page.",
    "Lead each answer with the answer. Move the context, the caveats, and the anecdote below the sentence that actually resolves the question.",
    "Do not add FAQPage or HowTo JSON-LD expecting a rich result: Google stopped showing the FAQ feature on May 7, 2026 and dropped HowTo back in September 2023.",
    "Do not reach for QAPage as a workaround. Google documents it for pages where users can submit answers, which a site-authored FAQ is not.",
    "For a pSEO template, render the questions and answers from the same per-record data, so each URL gets its own specific pair instead of one boilerplate block.",
    "Never ship boilerplate Q&A where only the entity name is swapped: identical questions across every page read as mass production to the same systems you want quoting you."
  ],
  spamBrainContext:
