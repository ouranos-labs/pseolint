...
// Lean rule knowledge baked into the MCP package (ships via dist) so the server
// can expose it as MCP resources without importing the web app.
const MCP_KNOWLEDGE = join(import.meta.dir, "..", "packages", "mcp", "src", "okf-knowledge.ts");
...
function main() {
  ...
  // Lean knowledge module for the MCP server (actionable subset only — no
  // marketing prose). JSON.stringify produces valid TS string/array literals.
  const knowledge = MARKETING_RULES.map((r) => ({
    ruleId: r.ruleId,
    slug: r.slug,
    category: categoryOf(r.ruleId),
    title: r.title,
    oneLiner: r.oneLiner,
    whatItDetects: r.whatItDetects,
    howToFix: r.howToFix,
    url: `${SITE}/rules/${r.slug}`,
  }));
  writeFileSync(
    MCP_KNOWLEDGE,
    `// AUTO-GENERATED by scripts/gen-okf.ts from apps/web MARKETING_RULES — do not edit.\n` +
      ...
      `export const RULE_KNOWLEDGE: RuleKnowledge[] = ${JSON.stringify(knowledge, null, 2)};\n`,
  );

  console.log(`OKF bundle: ${ruleFiles} rules in ${byCat.size} categories → ${OUT}`);
  console.log(`MCP knowledge: ${knowledge.length} rules → ${MCP_KNOWLEDGE}`);
}
