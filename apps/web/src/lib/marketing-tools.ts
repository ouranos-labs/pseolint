...
export const MARKETING_TOOLS: readonly MarketingTool[] = [
  {
    ...
    metaDescription:
      "Audit your site against 35+ inferred SpamBrain signals in 60 seconds. No signup. Spot scaled-content, doorway, and reputation-abuse patterns before Google does.",
    shortPitch:
    ...
    ruleLens: "spam/*",
    what:
      "The SpamBrain checker runs the full pseolint rule set — 42 rules across 8 categories, including the 8 dedicated spam/* rules — against a sample of your site, focused on the structural signals Google's SpamBrain classifier has been documented or strongly suspected to weight. We crawl your sitemap, fetch up to 100 pages on the free tier, and report every page-level finding plus a domain-level risk score from 0 (clean) to 100 (almost certainly being demoted). Median audit time is 60 seconds. The audit is read-only, respects robots.txt, and doesn't require you to install a tag, give us GSC access, or sign up.",
    why:
      "SpamBrain originally launched in 2018 and was rebuilt on a new ML stack in 2022, becoming Google's primary spam detection system. It has been the enforcement engine behind two of the most aggressive policies of the last two years: the March 5, 2024 scaled content abuse update and the May 7, 2024 site reputation abuse policy (https://developers.google.com/search/docs/essentials/spam-policies). Both targeted programmatic patterns that used to rank — automated city pages, AI-spun product comparisons, third-party content rented onto reputable subdomains. The checker exists because most pSEO operators only learn they crossed the line after a manual action notice or a 70% traffic drop. By that point you're rebuilding from scratch. Auditing structural signals before Google scores them is the only way to ship pSEO at scale without playing reactive defense.",
    howItWorks: [
      "Paste any public URL — the homepage, a hub page, or a representative template page works equally well.",
      "We pull your sitemap.xml and sample up to 100 URLs on the free tier, weighted toward URL patterns that look templated (high-cardinality path segments are oversampled).",
      "Each fetched page is parsed and run through 42 rules across 8 categories — including the 8 spam/* SpamBrain-adjacent rules covering thin content, doorway patterns, scaled boilerplate, internal-link cliques, and reputation-abuse signals.",
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "No. We send standard GET requests with a clearly identified user agent (pseolint/0.3 +https://pseolint.dev/bot), respect robots.txt and Crawl-delay, cap concurrency at 5, and stop at 50 pages or 50 MB total. Your analytics won't see the traffic and Search Console won't flag anything. Audits are read-only.",
      },
      {
      ...
      },
      {
        ...
        a: "The 50-page sample is weighted to oversample templated URL patterns, so you'll usually see your worst clusters even on huge sites. That said, sampling is lossy — a single bad template that lives in a tiny corner of the sitemap can be missed. If you need full coverage, the Pro plan audits up to 1,000 pages per run and supports scheduled monitoring.",
      },
      {
      ...
      },
      {
        ...
        a: "Yes. The full rule set lives at github.com/ouranos-labs/pseolint under the MIT license as the @pseolint/core package (v0.3.3) — you can run it locally with the CLI (`npm i -g pseolint@0.3.1`), audit your CI builds, or fork the rules. The hosted checker on this page is the same engine wrapped in a sampler and a UI.",
      },
    ],
  ...
  },
  {
    ...
    what:
      "The thin-content scanner samples your sitemap, fetches each page, and grades it against the substance heuristics SpamBrain appears to use: visible word count adjusted for template chrome, lexical uniqueness compared to sibling pages, presence of original media or sourced data, and the ratio of unique to recycled phrasing. It produces a per-page substance score plus a domain-wide breakdown of how much of your indexable surface area is below the practical threshold where Google starts ignoring or demoting pages.",
    why:
    ...
    howItWorks: [
      "Sample up to 100 URLs from your sitemap on the free tier, with extra weight given to URL patterns that look mass-generated. Median crawl + audit time is 60 seconds.",
      "For each page, strip nav, footer, and template chrome to isolate the actual unique main content.",
      "Score each page on visible word count (post-strip), lexical diversity, sentence-level uniqueness vs sibling pages, and presence of structured data, media, or citations.",
      "Cross-compare pages within the same URL pattern to detect near-duplicate boilerplate where the only variation is a swapped noun or city name.",
      "Surface the worst offenders first, with a substance score and a one-line diagnosis (under-length, near-duplicate, AI-padded, no unique research).",
    ],
    whatYouGet: [
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "Usually no — recovery from a quality-signal hit takes weeks to months, because Google needs time to re-crawl and re-evaluate the affected pages. But the alternative is that nothing improves. Fixing thin content is necessary but not sufficient: you also need Google to recrawl, re-render, and re-score, which is why you should pair content fixes with a sitemap resubmit and a few high-quality external links pointing at the recovered pages.",
      },
    ],
  ...
  },
  {
    ...
    what:
      "The doorway-page detector identifies clusters of pages on your site that match the structural definition Google uses in its doorway policy: pages that exist primarily to rank for query variants and funnel users to the same destination, distinguished only by a swapped city, service modifier, or product noun. It crawls your sitemap, groups URLs by template, and compares the actual rendered content within each cluster. You get a list of doorway-shaped clusters ranked by risk, with the option to see exactly which pages would survive a doorway-policy enforcement and which wouldn't.",
    why:
      "Google's doorway pages policy (https://developers.google.com/search/docs/essentials/spam-policies#doorway-pages) was formalised in a March 2015 Search Central post and has been on the books ever since. Enforcement accelerated sharply after SpamBrain was rebuilt in 2022 (the original classifier shipped in 2018) and then again after the March 5, 2024 scaled-content-abuse update extended doorway-style demotions to AI-spun funnels. Manual actions for doorway pages are rare — what happens instead is algorithmic demotion of the entire cluster, sometimes the entire site, with no notification. Programmatic SEO is particularly exposed because the same cost-saving template that lets you ship 50,000 pages overnight is also the structural fingerprint the policy was written to demote. The detector exists to draw the line between programmatic-but-substantive (a template that genuinely varies useful information per page) and programmatic-but-doorway (a template where the only variation is the keyword you're trying to rank for).",
    howItWorks: [
