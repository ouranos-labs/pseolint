...
export async function runCli(
  ...
  program
    ...
    .option("--crux-max-lookups <n>", "Cap on per-URL CrUX lookups; pages beyond it use origin-level field data (default: 150).")
...
async function runAudit(
  ...
  if (cruxApiKey) {
    const maxLookups = opts.cruxMaxLookups ? Number(opts.cruxMaxLookups) : undefined;
    cliFlags.crux = {
      apiKey: cruxApiKey,
