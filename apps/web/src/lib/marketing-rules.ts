...
export const MARKETING_RULES: readonly MarketingRule[] = [
  {
    ...
    oneLiner:
      "300 words is the default floor: pseolint counts substantive words per URL (after stripping nav and footer chrome) and flags every page below the threshold you set.",
    whatItDetects:
  ...
  },
  {
    ...
    oneLiner:
      "3 independent signals must converge — near-duplicate, entity-swap, and at least one further structural confirmation — before the rule fires at error severity (weight 25), the highest-confidence spam pattern pseolint reports.",
    whatItDetects:
  ...
  },
  {
    ...
    oneLiner:
      "0.85 SimHash similarity is the pseolint default threshold — every page pair at or above that mirrors Google's near-duplicate canonicalisation ceiling and is surfaced with the exact percentage for triage.",
    whatItDetects:
      "0.85 SimHash similarity is the threshold pseolint flags page pairs at, mirroring the near-duplicate canonicalisation ceiling Google's web indexing team has used since adopting Charikar's 2002 SimHash paper in 2007 — and named again in the March 5, 2024 scaled-content-abuse policy (https://developers.google.com/search/docs/essentials/spam-policies). For each page, the rule computes a 64-bit SimHash from the main content text using token-level shingling. It then compares every page against every other page — an O(n²) sweep that is fine for the page counts pSEO sites actually run (the 100-page free-tier audit ceiling completes in under a second; the 500-page Pro ceiling stays within budget). Hamming distance between two hashes is converted to a similarity score in [0,1]. Any pair scoring at or above the configured threshold (default 0.85, escalated to 0.90 for template-heavy sites) is recorded both as a finding and as a `PairMatch` consumed by `spam/doorway-pattern`. The finding fires at warning severity (weight 12) and includes the exact similarity percentage so you can sort the queue worst-first.",
    whyItMatters:
