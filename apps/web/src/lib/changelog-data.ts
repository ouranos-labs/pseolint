// GENERATED by scripts/gen-changelog.mjs. Do not edit directly.
...
export const CHANGELOG_PACKAGES: PackageChangelog[] = [
  {
    "name": "@pseolint/core",
    "channel": "npm · engine",
    "version": "0.8.1",
    "entries": [
      {
        ...
        "changes": [
          ...
          "**`links/generic-anchor-text`** (info): ≥50% of a page's internal links anchored on \"click here\"/\"read more\"/empty text. Also: `tech/og-completeness` now checks the two remaining ogp.me-required tags (`og:type`, `og:url`) at info severity; `CORE_RULESET_VERSION` bumped so change-driven monitoring re-fetches previously-skipped URLs. (This batch bumped it to 16; two later rules in the same release, `tech/resource-weight` and `content/image-attributes`, bumped it again, so the value this release ships is **18**.)",
          "New `content/image-attributes` rule: the parse-time half of the image blind spot, complementing `content/image-alt-text` (alt presence) and `tech/resource-weight` (image bytes under `--render`). No network access; everything comes from HTML the crawler already holds. Two independent signals. Missing dimensions fires at warning when at least half a page's `<img>` tags declare neither width/height attributes nor inline sizing, info below that: without an aspect ratio the browser cannot reserve space, which is what Cumulative Layout Shift measures. Inline `width`, `height` or `aspect-ratio` in a style attribute counts as sized, so CSS-driven layouts are not reported as broken. Responsive candidates fires at info when a page serves three or more images and not one uses `srcset` or `<picture>`; Google recommends responsive images, so this never rises above info and the message says it is guidance rather than a requirement. `loading=\"lazy\"` is deliberately not checked. Lazy-loading the LCP image actively harms it, so a correct verdict needs to know which image is above the fold, which static HTML cannot say; a rule flagging missing `lazy` everywhere would be wrong exactly where it matters most. There is no documented image count, dimension or byte limit, and this rule must never grow one. `CORE_RULESET_VERSION` is bumped to 18: a new scored rule changes findings on already-audited pages, so change-driven monitoring must re-fetch URLs it would otherwise skip.",
          "New `tech/resource-weight` rule, closing a real hole in `tech/html-size`. Googlebot's documented crawl cutoff is 2 MB per FETCHED FILE, so a 2.4 MB `bundle.js` is truncated exactly like a 2.4 MB HTML document would be. `tech/html-size` measured only the HTML while citing that doc, which meant an operator reading the finding could reasonably assume their scripts and stylesheets had been checked. They had not been. The new rule errors on any single subresource at or past 2 MB and warns within 25% of it. It also reports total page weight with a per-kind breakdown (image / script / stylesheet / font / other) at info severity. Read that finding's wording before turning it into a policy: Google documents no total-page-weight and no total-site-size crawl limit, so the number is reported as a Core Web Vitals input and the message says so explicitly. The 5 MB trigger is a reporting floor, not a published threshold (see `docs/folklore.md` #4). Byte totals come from the browser's Resource Timing buffer, read in the same `--render` pass that already collects LCP and CLS, so this costs one extra `page.evaluate` and zero extra network requests. `RenderedPage.resources` and `ParsedPage.resources` are new optional fields. Both are absent outside `--render`: subresource bytes cannot be known without fetching the assets, and the rule will not issue speculative requests to invent a number. Totals under-report rather than guess, since `transferSize` reads 0 for cross-origin responses without `Timing-Allow-Origin`. `CORE_RULESET_VERSION` is bumped to 17: a new scored rule changes findings on already-audited pages, so change-driven monitoring must re-fetch URLs it would otherwise skip.",
      ...
      },
      {
        "version": "0.7.3",
        "changes": [
          "v0.7.3: render-aware checks, AI content-effort, and bring-your-own inputs. Verdict moderators never change the raw `risk` number, so CI gates keyed off `--ci-threshold` stay stable.",
      ...
      },
      {
        ...
        "changes": [
          ...
          "**aeo/crawler-access**: honor robots `Allow` directives per RFC 9309 (allow-all no longer reported as fully blocked).",
          "**Severity/confidence bands**: error/critical demoted to warning on weak or forecast signals: thin-content medium band, summary-bait, translation-no-op, entity-swap (low mask coverage), soft-404 (OR-weighted confidence model). Note: bundled as a patch (0.x) despite a behavior/scoring shift and the `rules.uniqueValueMinWords` → `rules.uniqueValueDensity` config rename."
        ]
      },
      {
        "version": "0.7.0",
        "changes": [
          "Add `checkOriginHealth()`: a pre-flight origin probe that runs before an audit crawls. `BackpressureMonitor` only protects an origin _during_ a crawl, after dozens of requests have already landed on a struggling server (the paperforge/Neon incident, where each fetch fanned out into uncached DB queries that exhausted the egress quota). `checkOriginHealth()` fires a handful of **concurrent** probes at the entry URL first: concurrent, so it observes the origin the way the real crawl hits it (parallel fan-out), not a rosier one-request-at-a-time picture, and returns an `ok` / `unreachable` / `degraded` verdict.",
      ...
      },
      {
        "version": "0.6.6",
        "changes": [
          "Sitemap coverage guardrail + axis-aware unique-value guidance (fixes #3, #4) **#4: declared-vs-discovered coverage guardrail.** When a sitemap is found at discovery, two independent under-coverage signals now flag the run `truncated` with `truncatedKind: \"coverage\"` (distinct from the backpressure `\"backpressure\"` kind), reusing the existing partial-coverage surface (CLI/Action/MCP/web):",
          "**(A) unreachable child sitemaps.** `collectUrlsFromSitemap` now reports child total/failed counts, so a sitemap **index** whose children 404 / aren't valid sitemaps / exceed the depth cap is flagged: the case a URL-count comparison can never see, and the original false-negative class.",
          "**(B) fetch shortfall.** Far fewer pages were FETCHED than the sitemap declares. Compared against pages actually fetched (pre-filter, pre-sample) and bounded by every deliberate limit (explicit `--sample-size`, crawl cap, declared total), so noindex / non-HTML pages, intentional sampling, and a small crawl cap do **not** false-fire (the two false positives the first cut shipped with). Adds `AuditSummary.truncatedKind: \"backpressure\" | \"coverage\"` (+ JSON schema) so consumers and CI can branch on the cause rather than overloading one boolean. **#3: `content/unique-value` guidance.** The fix string is now axis-aware (warns that content shared across same-axis sibling pages (boilerplate, per-axis data) does NOT count), the message surfaces the shared-vs-unique word split, and tokenization strips surrounding punctuation so `\"word\"` / `\"word.\"` count as one token (removing false precision in the surfaced counts). Also routed a remaining `splice(0, n, ...big)` spread through the iterative `pushAll` helper (same V8 argument-cap class as the earlier crash fix).",
          "A truncated run can no longer present as a confident clean verdict Found by re-running `pseolint https://paperforge.dev` against the live site: the backpressure watchdog (correctly) aborts the cold-start origin after ~11 fetches, but the 1-page salvage was then run through the normal classify → score → verdict pipeline and emerged as `small-marketing` + suppressed pSEO rules + **`READY ✓`**, reproducing the original case-study false-negative via the watchdog rather than via discovery (which works). Two fixes:",
          "**Classification:** when a run is truncated BEFORE classification (a backpressure abort salvaged only a fragment), the site type is forced to `unclear` (confidence 0, no rule suppression). Classifying a salvaged fragment as a confident `small-marketing` site (and suppressing the pSEO rules off it) is what produced the false green.",
          "**Verdict:** any truncated run's verdict is floored to at least `caution`: it can never read `ready`, so the headline matches the partial-coverage banner. The watchdog itself is unchanged: it exists to protect exactly this origin (its crawl fans out into uncached DB queries), so it should keep aborting; the fix is that the salvaged report is now honest about being incomplete."
        ]
      },
      {
        "version": "0.6.5",
        "changes": [
          "Harden the public JSON output contract. Bump `SCHEMA_VERSION` from `2026-04-v0.4` to `2026-06-v0.6` so it reflects the current v0.6 shape (`templates[]`, `truncated`/`truncatedReason`); the constant had drifted and was never bumped when those landed. Publish a draft 2020-12 JSON Schema at `schemas/audit-summary.schema.json` (now shipped via the package `files`/`exports`) and add a schema-contract test that validates the JSON formatter output against it so the schema and types can't silently drift. Document the contract in the README, including that `issues` is severity-bucketed (`blockers`/`shouldFix`/ `informational`) rather than a flat array or category-keyed map, that `schemaVersion` bumps on every breaking or additive-public output change, and that `truncated: true` means counts/verdict are lower bounds for CI gates.",
          "flush a partial `truncated:true` report on watchdog abort + warn on localhost concurrency override When the backpressure watchdog aborted a crawl mid-flight (the real user run at `--concurrency 5` against a single-process dev server), the `OriginDegradedError` propagated out of `auditSource()` and every downstream phase (dedup, rules, enrichment, scoring, report assembly) was skipped; the CLI caught the error, printed \"aborted: origin looks degraded\", and exited 1 with **zero output**. Protecting the origin is correct; throwing away everything collected is not. `loadPagesFromSource()` now fills a caller-owned salvage sink incrementally, so a mid-crawl abort no longer loses the pages already fetched. `auditSource()` catches the watchdog abort at the page-loading boundary (and at the next abort checkpoint, for fetch implementations that ignore the abort signal), recovers the partial page set, runs the rest of the pipeline over it, and returns a normal `AuditSummary` with `truncated: true` and `truncatedReason` set to the origin-degraded message. A zero-page abort still returns a valid truncated summary instead of crashing. External aborts (ctrl-C / parent timeout) and `--no-backpressure` are unchanged. The CLI now prints a clear `⚠ PARTIAL REPORT` banner to stderr for a truncated run, still writes/emits the report (JSON/console/etc.), and exits 1 so CI knows coverage was incomplete. It also warns (without changing the value) when a localhost/single-origin target is crawled with an explicit `--concurrency` greater than the dev preset's 1, suggesting `--concurrency 1`.",
          "Sitemap-first discovery for homepage/page sources (closer to how Google crawls) Previously, auditing a homepage URL only link-crawled; it never read the site's declared sitemap, so a programmatic site with thousands of sparsely-linked (or build-frozen, under-linked) URLs was under-discovered. Nested `<sitemapindex>` files were already followed when a sitemap URL was passed directly, but nothing _discovered_ the sitemap from a plain page URL. The HTML-source path now does sitemap-first discovery before link-crawling: 1. reads `Sitemap:` directives from robots.txt (there can be several), and 2. failing that, probes `/sitemap.xml` then `/sitemap_index.xml`. Discovered sitemap URLs are SSRF-validated, same-origin- and robots-Disallow- filtered, then fetched first (they're authoritative); the existing link-crawl then fills any remaining discovery budget and dedups against them. When no sitemap exists this is a no-op and the audit link-crawls exactly as before. Hardening: `<sitemapindex>` recursion is now depth-capped (5 levels) in addition to the existing `visited` cycle guard, and individual sitemap fetches are capped at 50 MB (the sitemaps.org limit) so a hostile/misconfigured sitemap can't eat the byte budget. `fetchRobotsMeta` now also returns the parsed `Sitemap:` directives. Deferred (noted, not in this change): `.xml.gz` sitemaps served as `application/gzip` without `Content-Encoding: gzip` (the common gzip case via `Content-Encoding` already works through fetch), URL normalization before sitemap sampling, and running the change-driven monitoring matrix over discovered sitemaps on homepage-sourced audits.",
          "make cluster/link-graph traversal iterative to fix \"Maximum call stack size exceeded\" on large sites On a large, densely cross-linked site (the 351-page reproduction), report generation crashed with `RangeError: Maximum call stack size exceeded` and produced zero output. Clusterable rules (`spam/near-duplicate`, `spam/entity-swap`, `spam/doorway-pattern`) emit one finding per related pair, so a single fully-connected component yields C(N,2) pairwise findings. While collapsing a component into one cluster finding, `enrichFindings` computed the similarity range with `Math.min(...similarities)` / `Math.max(...similarities)`. The spread operator passes every array element as a separate call argument, and V8 caps the argument count (~131072), so a similarity array of hundreds of thousands of pairs overflowed the call stack. The similarity range is now computed with a single iterative pass, and the post-grouping `passthrough.push(...groupedPassthrough)` spread was replaced with an element-wise loop for the same reason. Output is unchanged. Added a deterministic, synthetic regression test (`tests/enrich-findings-large-cluster.test.ts`) that builds a 600-node fully-connected near-duplicate component (179,700 pairs) and asserts `enrichFindings` no longer throws. Follow-up: the same `push(...spread)` overflow existed earlier in the pipeline, which the enrichment-only test missed. In `auditSource` the rule-aggregation steps spread the per-rule findings into the running list (`findings.push(...tag(rule.findings))` and `allFindings.push(...)`), so a dense site overflowed at rule aggregation _before_ enrichment was reached. All such spreads in the audit path are now routed through an iterative `pushAll` helper (rule aggregation, sitemap-URL and discovered-page accumulation), plus the latent `Math.max(...counts.values())` in the site classifier (100k+ distinct titles) and the spreads in `stratified-sample` and the AI `fetch-sitemap` tool. Added an integration test (`tests/integration/large-corpus-no-overflow.test.ts`) that drives the full `auditSource` over a 600-page directory corpus; it first asserts the guarded-against spread still overflows on the runtime, then that the audit completes. This is the test that caught the second (pre-enrichment) site.",
          "fix unscoped title selector picking up SVG <title>; add SVG-title diagnostic `parseHtmlPage` extracted the page title with an unscoped `$(\"title\")` selector, which also matches inline SVG `<title>` elements (e.g. a logo's accessibility label). On pages with no `<head><title>` but an SVG logo, the SVG label was reported as the page title, the cause of \"307 podcast episodes all titled Spotify\". Title extraction is now scoped to `$(\"head > title\")`. The parser records `titleSource` (`\"head\"` or `\"none\"`) and, when the head title is missing, captures the first inline SVG `<title>` text as `svgTitleSample`. The `content/title-uniqueness` rule uses these to emit a diagnostic naming the SVG-title trap instead of a generic missing-title message."
        ]
      },
      {
        "version": "0.6.4",
        "changes": [
          "Export `SCORED_CATEGORY_KEYS`: the canonical list of verdict-contributing category keys (every `CategoryKey` except the weight-0 `audit` diagnostics bucket). Gives downstream consumers (MCP, CLI, web) a single source of truth for \"the scored categories\" instead of each hardcoding `[\"integrity\", clause keeps it in sync if a key is renamed."
        ]
      },
      {
        "version": "0.6.2",
      ...
      },
      {
        ...
        "changes": [
          ...
          "**Verdict/grade alignment** in dashboard layer (`@pseolint/web` 0.0.7): band labels now align with the engine's verdict ladder so a \"B / caution\" visual mismatch is impossible. Net effect on bestfirenze.com: classification `small-marketing` → `unclear`, severity demotions removed, blocker count climbs, density floor applies; risk lands ≥60 (D / critical) with verdict and grade reading the same vocabulary."
        ]
      },
      {
        "version": "0.5.0",
        "changes": [
          "v0.5.0 (continued): AI orchestrator: 25 tools, fix-manifest output, validators + diffs Net-new public API: `orchestrate(opts)`, which drives an LLM through 25 deterministic tools (sitemap fetch, template clustering, per-page rule checks, AEO probes against live Anthropic/Perplexity/Gemini, SerpAPI) and produces a **fix manifest** of concrete patches (`replace_h1`, `rewrite_meta`, `add_jsonld`, `add_faq_block`, `rewrite_intro`, `add_internal_link`, `remove_thin_block`, plus domain-level `robots_txt`/`sitemap_xml`/`canonical_strategy`). **Architecture**: rules become tools the LLM calls; the LLM picks order; budget caps (LLM tokens + external probe USD, pre-flight + reactive enforcement) bound spend. Watchdog injects a convergence reminder every N tool calls. Page cache by reference (HTML never travels in conversation history) keeps token cost bounded as audits scale. **Manifest validation** (Phase 4): 11 deterministic patch validators (Schema.org required-properties, robots.txt structure, sitemap XML, cheerio HTML safety with allowlisted tags, etc.) run on every LLM-proposed patch. Failed patches are dropped from the manifest and surfaced in `validation.failures` with structured location info; the LLM never gets the chance to ship a malformed JSON-LD block or an unsafe `<iframe>` to a user. **Structured diffs** (`diffManifest`): every patch maps to one of 5 `PatchDiff` kinds (text_replace, html_insert, html_remove, file_replace, guidance) suitable for direct UI rendering with HTML-escaped attributes. **External probe tooling**: `query_serp` (SerpAPI, $0.005/call), `ask_ai_engine` (Anthropic / Perplexity Sonar / Gemini citation probes), `validate_jsonld`, `check_robots`, `check_indexability`, `check_domain_llms_txt`, `check_domain_crawler_access`. All cost-tracked. Public exports: `orchestrate`, `runOrchestrator`, `orchestratorTools`, `defineTool`, `validateManifest`, `diffManifest`, `manifestSchema`, `buildSystemPrompt`, `DEFAULT_BUDGET`, plus types `FixManifest`, `BudgetCaps`, `UsageSnapshot`, `StopReason`, `SessionEvent`, `SessionResult`, `PatchDiff`, `ManifestDiff`, `ManifestValidationReport`. AbortSignal threading through every I/O tool. AsyncLocalStorage-backed page cache. Verified end-to-end across 4 dogfood runs against pseolint.dev (final run: 36 tool calls, $0.55, 4 minutes wall, completed manifest with 5/6 patches passing validators; one rejected meta-description for being 167 > 160 chars).",
          "v0.5.0: Change-driven monitoring **Why:** Monitoring runs on a 4k-page site re-fetched everything. Rule evaluation is microseconds; the fetch is seconds. The pre-v0.5 `--since` flag did change-detection at the wrong layer: it post-filtered findings on already-fetched pages, paying the network cost on every URL just to skip a few microseconds of CPU. v0.5 moves the decision upstream of the fetch so unchanged URLs are never network-touched. **Architecture:** New `planScrapeStrategy()` pure module evaluates each candidate URL against a 7-reason decision matrix BEFORE fetching: new, age floor (default 7d), ruleset version mismatch, open findings recheck, sitemap `<lastmod>` newer than prior fetch, GSC delta (Pro), or no skip evidence. URLs that match a refetch reason are fetched as today; URLs that match `unchanged` are skipped entirely and their findings are carried forward from prior state with `carriedForward: true` and `lastVerifiedAt` markers. Expected savings depend strongly on sitemap hygiene:",
          "**Sites with `<lastmod>` in sitemap.xml** (Next.js, WordPress/Yoast, Astro): up to ~95% fetch reduction on steady-state monitoring runs once the prior state has aged past the recheck-trigger findings.",
          "**Sites without `<lastmod>`** (custom-rolled sitemaps, older CMSes): every URL hits the `no-signal` reason and gets refetched. v0.5 monitoring helps via faster cache revalidation but doesn't skip the round-trip. A future HEAD-fallback path (deferred) will close this gap. Severity gate on the `recheck` reason: only `error`, `critical`, and `warning` findings trigger a per-run recheck. `info` findings carry forward without re-fetching the page. Without the gate, any URL with any finding would refetch and the carry-forward path would be dead code. **Breaking:**",
          "**State schema bumped to v2.** Existing `.pseolint/state.json` files from v0.4.x are discarded with a warning on first read; users get one full baseline audit, then incremental monitoring kicks in.",
          "**Auto-monitoring is the new default** when a prior state file exists. Pre-v0.5 required `--since` to opt in. Use `--mode=fresh` to force a full re-audit even with prior state present. **Added:**",
          "`planScrapeStrategy()` exported from `@pseolint/core`: pure decision matrix; testable without I/O.",
          "`CORE_RULESET_VERSION` constant. Bump when adding rules or materially changing rule logic so monitoring runs re-evaluate previously-skipped URLs against the new ruleset.",
          "`AuditSummary.scrapePlan` reports `fetched` / `carriedForward` counts, per-reason breakdown, ruleset version, and last full audit timestamp.",
          "`RuleResult.carriedForward` and `RuleResult.lastVerifiedAt` mark findings carried over from a prior run for staleness reasoning.",
          "`UrlStateEntry.findings` now persists full RuleResult records (not just IDs) so future runs can carry them forward.",
          "`parseSitemapUrlsWithLastmod()` exported: sitemap walker now surfaces `<lastmod>` alongside URLs.",
          "CLI: `--mode=monitoring|fresh` and `--age-floor-days=N`. **Changed:**",
          "`--since` is now an alias for `--mode=monitoring` (kept for back-compat). Behavior is unchanged for users who passed it explicitly.",
          "`collectUrlsFromSitemap` returns `{ urls, lastmodByUrl }` instead of `string[]`. Internal API; no external consumers.",
          "`RunState` adds required `rulesetVersion` and `lastFullAuditAt`. See spec: `docs/superpowers/specs/2026-05-01-change-driven-monitoring-design.md`."
        ]
      },
      {
        "version": "0.4.3",
        "changes": [
          "v0.4.3: classification-driven scoring, fixes credibility crisis Trigger: an external reviewer ran pseolint on his own site and didn't believe the verdict. Three-site dogfood (nextjs.org, wordpress.com, shopify.com) confirmed the issue: nextjs.org would score `concerning` (60) on a meaningful sample, with 7 of 11 actionable findings being AEO-style \"your marketing page doesn't read like a fact database.\" Citation category alone drove 25 risk points just from the AEO bucket maxing out. This release rewires scoring so the verdict is **classification-aware**: a docs site is judged by what matters for docs sites; a programmatic directory is judged by what matters for pSEO; a marketing site isn't penalized for being marketing-shaped. **Changes:**",
          "New `SCORING_PROFILES` map keyed on `SiteType`. Each profile defines per-category weights + severity overrides + confidence overrides. Applied when classifier confidence ≥ 0.7; below that, conservative `unclear` defaults.",
          "New `RULE_IMPACTS` map gives every rule its own `baseImpact + perInstance × maxImpact` curve, replacing the global 4-tier `SEVERITY_WEIGHTS`. Spam findings amplify with cluster size; AEO findings stay capped low.",
          "New `Confidence` type (`high | medium | low | speculative`) on `RuleResult`. Per-rule emit logic on 10 rules. Low-confidence findings carry a caveat in the message and contribute proportionally less to scoring.",
          "New site classifier types: `docs` (Docusaurus / Nextra / GitBook / VuePress shape) and improved `ecommerce` (Shopify / WooCommerce shape). Plus a `tryClassifyLocalizedMarketing()` detector that prevents stripe.com / vercel.com / linear.app from being misclassified as `programmatic-directory` because of `/[lang]/` URL prefixes.",
          "Re-tuned tech rule impacts: `tech/hreflang-consistency` is now a single base-impact finding regardless of how many language pairs break (one declaration breaks them all; count shouldn't compound). `tech/canonical-consistency` lowered to base 8, perInstance 1.",
          "Formatters surface \"Audited as &lt;type&gt; (NN% confidence).\" prominently. Confidence caveats render after low-/speculative-confidence findings.",
          "Marketing copy on /, /tools, /rules, /symptoms, /limits clarifies scope: \"pseolint audits programmatic-SEO + AI Overview readiness. Not a general SEO audit; for Core Web Vitals and broken links use Sitebulb, Screaming Frog, or Ahrefs.\" **Dogfood results (post-change):** ``` nextjs.org      ready (14)   → ready (15)   no regression react.dev       caution (23) → ready (15)   improved stripe.com      [aborted]    → ready (9)    correctly classifies localized wordpress.com   caution (24) → ready (12)   improved shopify.com     concerning(58)→ caution(28) -30 risk; remaining findings real ``` Tests: 663/663 pass (was 646 in v0.4.2; +17 new tests for classifier types, scoring profiles, confidence emission, formatter output). **Reasoning trail:** see `docs/superpowers/specs/2026-04-30-pseolint-scoring-credibility.md` for the full diagnosis + design rationale."
        ]
      },
      {
        "version": "0.4.2",
        "changes": [
          ...
          "`skipBoilerplate?: boolean` (default `false`): skip cookie / legal / consent / imprint pages via title, H1, or URL pathname matching well-known compliance-page patterns (`/privacy`, `/terms`, `/cookies`, `/gdpr`, `/ccpa`, `/impressum`, `/disclaimer`, `/accessibility`, `/do-not-sell`, etc.). Single-signal trigger because patterns are anchored: a marketing page that mentions \"privacy\" in its body won't fire. New `detectBoilerplatePage(page)` exported from `./page-filter.js`.",
          "`skipSearchPages?: boolean` (default `false`): skip pages with search-result URL hallmarks: query parameter `q` / `query` / `search` / `s` / `keyword`, or pathname starting with `/search`. Per Google's own guidance these should be `noindex`'d but many sites don't tag them.",
          "`skipEmptyBody?: boolean` (default `false`): skip un-hydrated SPA shells: body text < 100 chars, script tags present, no substantive `<noscript>` fallback. The right fix is `--render`, not content rules. `pageSkipReason()` now returns `\"noindex\" | \"auth-detected\" | The `audit/skipped-by-policy` diagnostic surfaces all five reason categories with per-reason counts. Plus a refactor for output ergonomics:",
          "New `bucketByTemplate(findings)` helper at `./formatters/bucket-findings.js`. Console + markdown formatters now collapse findings sharing a template signature into one line (`× 23 instances on /templates/[state]-llc-fees the legacy format. Site-wide / non-template buckets render as `× 2 affected pages`.",
          "New `formatFixplan(summary)` formatter at `./formatters/fixplan.js`. Emits a markdown checklist of fixes ordered by effort (quick wins → moderate → structural → other), each item bucketed by template, with a Skipped section breaking down noindex / auth-detected counts and a footer wallclock estimate. Designed for paste-into-GitHub-issue use. **pseolint CLI (0.4.1 → 0.4.2)**",
          "New flags: `--skip-boilerplate`, `--skip-search-pages`, `--skip-empty-body` mirror the corresponding `AuditOptions`. All off-by-default to preserve CLI back-compat. **@pseolint/mcp (0.4.1 → 0.4.2)**",
          "Workspace dep bump to pick up the new `AuditOptions` fields. No tool surface changes; callers can pass the new flags via the MCP audit tool's options if they want them. **Web app (0.0.4 → 0.0.5)** The hosted form audit pipeline now adapts to the audited site's framework:",
          "New `FRAMEWORK_IGNORE_PATTERNS` map keyed on `nextjs | wordpress | idiomatic-but-non-marketing routes (e.g. WordPress `/wp-includes/`, Shopify `/cart`, Next `/_next/data/`, etc.) layer additively on top of the base `WEB_AUDIT_DEFAULT_IGNORE`.",
          "New `detectFrameworkFromUrl(url, signal?)` helper does a single preflight HEAD/GET of the source root, checks `x-powered-by`, `x-vercel-id`, `x-shopify-*` response headers + script-src body signatures, and returns the framework key. Wrapped in an `AbortController` with a hard 5s timeout so a slow root doesn't block the audit.",
          "`run-audit.ts` runs framework detection in its own Inngest step (`detect-framework`) before the audit step. Failure / timeout falls through to `resolveAuditIgnorePatterns(undefined)` (base list only). Tests: 646/646 pass. core + cli + mcp + action all build clean."
        ]
      },
      {
        "version": "0.4.1",
        "changes": [
          ...
          "New `respectNoindex?: boolean` (default `true`): pages explicitly marked `noindex` (via `<meta name=\"robots\">` or `X-Robots-Tag` header) are excluded from rule evaluation. The site owner already opted out of SEO indexing for them; auditing produces noise the reader can't act on. The two noindex-conflict rules (`tech/canonical-noindex-conflict`, `tech/robots-noindex-conflict`) and `tech/hreflang-consistency` still receive noindex'd pages so they can flag accidental noindex'ing / inconsistent hreflang declarations.",
          "New `skipDetectedAuth?: boolean` (default `false`): heuristic detection of login / signup / password-reset / verify-email pages via password-input density, page title pattern (brand-suffix-stripped), and H1 pattern. Requires 2+ signals for a positive verdict, keeping false-positives low on marketing pages with single-signal characteristics.",
          "New `audit/skipped-by-policy` diagnostic surfaces every URL skipped by the above policies in `summary.diagnostics.auditFindings`; the accidentally-noindex'd page now shows up as a visible skip line instead of being absent without explanation.",
          "New `warnUnmatchedIgnore?: boolean` (default `false`): per-pattern warning for unmatched `--ignore` patterns is now opt-in. The CLI sets it to true only when `--ignore` came from the command line. Config-loaded patterns (e.g. `pseolint.config.ts` with broad safety patterns like `**/api/**`) no longer spam warnings when the patterns legitimately don't match a small site's surface. A consolidated `none of the N ignore patterns fires when ALL patterns miss, regardless of source.",
          "New helpers exported from the entry point: `detectNoindex`, `detectAuthPage`, `pageSkipReason` from `./page-filter.js`. **pseolint CLI (0.4.0 → 0.4.1)**",
          "`pseolint.config.ts` files are now auto-loaded via cosmiconfig (jiti loader). Previously only `.js` / `.cjs` / `.mjs` / `.json` configs were picked up; `.ts` files silently fell through, forcing users to inline `--ignore` patterns. Both `pseolint.config.ts` and `.mts` variants are now in the searchPlaces list.",
          "New flag `--no-respect-noindex`: audit pages marked noindex anyway (useful when investigating an accidentally-noindex'd page).",
          "New flag `--skip-detected-auth`: opt into the heuristic auth-page skip. **@pseolint/mcp (0.4.0 → 0.4.1)**",
          "Picks up the core changes via the workspace dep bump. No tool surface changes; auth-page detection / noindex respect are now available to callers that pass the relevant `AuditOptions` fields. **Web app** Public-form audits run through `apps/web/src/inngest/functions/run-audit.ts` now apply opinionated defaults: a `WEB_AUDIT_DEFAULT_IGNORE` URL pattern list (framework metadata + auth + admin + API + WordPress conventions) plus `respectNoindex: true` and `skipDetectedAuth: true`. End users running audits via the public form no longer see noise from utility routes. Tests: 604 / 604 pass. Engine + CLI + MCP + action all typecheck clean."
        ]
      },
      {
        "version": "0.4.0",
        "changes": [
          ...
          "Replaced numeric `score` (0–100, lower = better) with the verdict ladder `ready` (≤20) | `caution` (≤40) | `concerning` (≤60) | `critical` (>60). The numeric value is still emitted as `risk` for sorting and CI gating.",
          "Consolidated 8 source-tree categories into 4 scoring super-categories: `integrity` (spam + content + cannibal, weight 0.50), `discoverability` (links + tech, 0.20), `citation` (aeo + schema, 0.25), `data` (0.05).",
          "Dropped 8 noisy rules (cannibal/title-overlap, cannibal/keyword-collision, content/heading-uniqueness, links/hub-pages, plus four others). Total rule count is now 32.",
          "New `AuditSummary` shape: `schemaVersion: \"2026-04-v0.4\"`, `verdict`, `risk`, `headline`, `categories[].grade/issues`, `issues.{blockers,",
          "New `siteClassification` field (§4.11): pre-flight URL-pattern + sitemap - framework heuristics infer `small-marketing | blog | programmatic- (`spam/template-coverage`, `spam/template-diversity`, `spam/entity-swap`, `cannibal/url-pattern`) on small sites unless the caller passes `strict: true`.",
          "New `AuditOptions.strict` flag to bypass classification-driven rule suppression. **pseolint CLI (0.3.2 → 0.4.0)**",
          "New flags: `--ci-threshold <severity>` (replaces numeric `--threshold` for CI gating), `--explain` (full bucketed finding view), `--strict` (bypass pSEO-only rule suppression).",
          "`--threshold` deprecated with a runtime warning: still functional for one minor.",
          "New `pseolint diff <baseline> <current>` subcommand: verdict-rank deltas + fixed/regressed/new findings between two AuditSummary JSON reports. Tolerates mixed v0.3 / v0.4 JSON.",
          "Console formatter rewritten around the verdict ladder. JSON, HTML, and Markdown formatters all consume the new shape.",
          "`--watch` flag reserved (planned for v0.4.1, not implemented). **@pseolint/mcp (0.3.1 → 0.4.0)**",
          "All three tools (`audit_site`, `explain_score`, `check_page_technical`) migrated to the v0.4 `AuditSummary` shape: `summary.score` → `summary.risk`, `summary.findings` → flattened from `issues.*`, `summary.categoryScores` → `summary.categories`.",
          "`explain_score` now surfaces `siteClassification` (type, confidence, suppressed-rule count) when present.",
          "`CROSS_PAGE_RULES` set updated to remove the 4 dropped cross-page rules. **Migration** Pre-existing v0.3 JSON reports remain readable; `/r/[slug]` and `/r/compare` in the web app detect `summary.schemaVersion` and route through legacy renderers. New CI gates should switch from `--threshold 40` (numeric) to `--ci-threshold concerning` (semantic). Spec: `docs/superpowers/specs/2026-04-29-pseolint-v0.4-engine-redesign.md`."
        ]
      },
      {
        "version": "0.3.3",
        "changes": [
          ...
          "`safeMode: \"saas\" | \"cli\"` preset on `AuditOptions`: flips `guardSsrf`, `respectRobotsTxt`, `followRedirects`, `maxCrawlDiscovered`, and `maxFetchBytes` defaults in one knob. Individual option overrides still win.",
          "`safeFetch(url, options?)`: SSRF-safe fetch for non-audit use cases (webhook URL verification, favicon lookups, etc.). Wraps `cachedFetch` with `validateTargetHost` baked in.",
          "`maxCrawlDiscovered`: hard ceiling on link-discovery fan-out so a malicious site with many self-links can't extend crawl up to the byte budget. Default 5000 (2000 under `safeMode: \"saas\"`).",
          "`followRedirects: false` option: returns 3xx as-is so security- sensitive audits can report redirects without following them. New exports: `safeFetch`, `SafeMode` type. ## pseolint (CLI) → 0.3.1 First CLI release since 0.3.0; bundles the v0.3.1 / v0.3.2 / v0.3.3 CLI-facing work:",
          "Render-mode analytics blocking flags: `--analytics <block|allow|allow-first-party>`, `--block-host <host>` (repeatable). Prevents rendered audits from firing GA / Plausible / PostHog / Mixpanel / Hotjar / Sentry beacons on every page.",
          "`--safe-mode <saas|cli>`: applies the core preset.",
          "`--no-respect-robots`: audit sitemap URLs even when the target's robots.txt Disallow's them (use for your own staging sites).",
          "`--no-follow-redirects`: report 3xx as-is.",
          "`ctrl-C` handler: SIGINT triggers a clean abort via `AbortController`; in-flight fetches cancel cleanly instead of the process being hard- killed mid-read. Second `ctrl-C` within ~1 s forces exit.",
          "`pseolint.config.ts` schema extended for `safeMode`, `respectRobotsTxt`, `followRedirects`, `guardSsrf`, `maxCrawlDiscovered`. ## @pseolint/mcp → 0.3.1 First MCP release since 0.3.0; picks up all of v0.3.1 / v0.3.2 / v0.3.3 via the core update, plus one meaningful default flip:",
          "All three tool handlers (`audit_site`, `explain_score`, `check_page_technical`) now default `safeMode: \"saas\"`. AI assistants running in end-user environments can't be tricked into scanning cloud metadata / localhost / RFC1918 networks via a malicious URL argument. `guardSsrf`, `respectRobotsTxt`, tighter caps all flip on. ## Not changed `@pseolint/action`: runs in GitHub-public-network runners, auto- propagates new core features through its existing `AuditSummary` rendering. No separate bump. ## Test state 528 / 528 tests pass. Typecheck clean across core / cli / mcp / action."
        ]
      },
      {
        "version": "0.3.0",
        "changes": [
          "feat: add AEO rule category with 8 rules for AI Overview readiness Introduces `aeo/*`, a new scored rule category focused on Answer Engine Optimization. Brings the total to **42 rules across 8 categories** (7 scored",
          "`data/*`). While SpamBrain rules protect against Google penalties, AEO rules audit whether pSEO pages are structured to be cited in AI Overviews (ChatGPT, Perplexity, Gemini, Claude, Google AI Overviews). Sites invisible to AI Overviews lose ~68% of traffic vs ~12% for cited sites. New rules:",
          "`aeo/llms-txt`: checks for `/llms.txt` at the origin and validates the minimal shape (H1 title, at least one `##` section, markdown link entries).",
          "`aeo/crawler-access`: parses `robots.txt` per user-agent and flags blocked AI crawlers (GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Bytespider, Google-Extended, CCBot, Applebot-Extended). Warns per blocked crawler, errors when all are blocked.",
          "`aeo/freshness-signals`: checks each page for a dateModified signal (JSON-LD, `article:modified_time`, visible \"Last updated\" text). Warns when absent, emits info when older than 180 days.",
          "`aeo/faq-coverage`: detects FAQ-style content (question-phrased H2s or URL patterns like `/how-to-*`, `/what-is-*`, `*-faq`) that lacks FAQPage or HowTo JSON-LD.",
          "`aeo/answer-first`: scores the first paragraph after the H1 for extractable-answer quality: concrete facts, named entities, complete sentence, boilerplate detection, and template-opener detection via entity masking across the corpus.",
          "`aeo/citable-facts`: counts unique, entity-specific citable facts per page (dollar amounts, percentages, timeframes, dates, Form numbers). Filters out template facts shared across the majority of masked pages.",
          "`aeo/non-replicable-value`: detects pages that are pure informational text with no interactive element, downloadable asset, or gated content: pages AI can fully summarize without sending a click.",
          "`aeo/content-modularity`: splits pages by H2/H3 and flags sections that aren't independently extractable (cross-references like \"as mentioned above\", vague headings like \"More Info\", paragraphs over 200 words). ### Scoring Category weights re-balanced. `aeo` joins the composite score at 15%: before (0.2.x): spam 0.40 / content 0.25 / links 0.15 / tech 0.10 / schema 0.05 / cannibal 0.05 after  (0.3.0): spam 0.35 / content 0.20 / aeo  0.15 / links 0.12 / tech 0.08 / schema 0.05 / cannibal 0.05 `AuditSummary.categoryScores` now includes an `aeo` field. ### Config New flat threshold keys in `AuditOptions.rules` (all optional, sensible defaults):",
          ...
          "`faqMinQuestionHeadings` (default 2): `aeo/faq-coverage` Page-group `overrides` still apply normally for severity tuning. ### AEO sub-score and console section `categoryScores.aeo` is the sub-score (raw 0–100 damage; lower is better) and has its own label scheme distinct from the SpamBrain Risk label:",
          ...
          "81–100 **Ghost**: blocked from AI + no citable structure; traffic will crater `aeoScoreLabel(score)` is exported from `@pseolint/core` so downstream formatters can surface the label. The console formatter renders a dedicated **AEO: AI Overview Readiness** section between group scores and AI triage when any `aeo/*` findings are present. ### AI triage Prompt bumped to `1.1.0` (additive). The system prompt now explicitly distinguishes two threat families: **SpamBrain penalty risk** (spam/cannibal/ content/data/tech/schema/links) and **AI Overview invisibility** (aeo/\\*), and asks for at least one root cause from each when both families are present. A new `findingCountByCategory` field in the prompt payload gives the model per-category totals for weighting.",
          "feat(core): diff-mode audits + per-rule scope declarations Adds a declarative `RULE_SCOPE` map at `rules/scope.ts` that marks every rule ID as either `\"page\"` (output depends only on a single parsed page) or `\"corpus\"` (needs the full set of pages: clustering, cross-page comparisons, the link graph, robots.txt). New `AuditOptions.mode`:",
          "`\"full\"` (default): runs all rules; identical to prior behaviour.",
          "`\"diff\"`: skips corpus-scoped rules so daily diff-audits can re-evaluate only changed pages without re-running clustering / link-graph / sitemap rules against the unchanged corpus. Consumed by the hosted monitoring pipeline (`apps/web` `inngest/functions/monitor-domains.ts`). Local CLI and MCP flows are unaffected; the default remains `\"full\"`. Public API additions:",
          "`RULE_SCOPE`: `Record<string, RuleScope>`, complete map of all 42 current rule IDs.",
          "`isRuleAllowedInDiff(ruleId)`: returns true for page-scoped rules; unknown IDs default to corpus (safer).",
          "`RuleScope` type: `\"page\" | \"corpus\"`. AEO rules are included in the scope map: `aeo/freshness-signals`, `aeo/faq-coverage`, `aeo/answer-first`, `aeo/citable-facts`, `aeo/non-replicable-value`, `aeo/content-modularity` are page-scoped; `aeo/llms-txt` and `aeo/crawler-access` are corpus-scoped (they need origin-level state). Note: `answer-first` and `citable-facts` perform cross-page template detection internally, but in diff mode they receive only the changed pages; template-fact and template-opener detection are effectively scoped to the diff set, not the historical corpus. This is acceptable for daily re-runs; full audits remain the source of truth for corpus-wide patterns."
        ]
      },
      {
        "version": "0.2.2",
        "changes": [
          "fix(publish): rewrite `workspace:*` to real semver in published dependencies The 0.2.1 tarballs shipped with `workspace:*` in their `dependencies` lists, which npm cannot resolve. Any `npx pseolint` or `npm install pseolint` silently failed. Republishing via `changeset publish` rewrites the workspace protocol to the real version range."
        ]
      }
    ]
  },
  {
    ...
    "entries": [
      ...
      },
      {
        "version": "0.5.0",
        "changes": [
          "v0.5.0 (continued): `pseolint orchestrate` subcommand Wraps `@pseolint/core` v0.5.0's new `orchestrate()` API as a CLI command. ```bash export ANTHROPIC_API_KEY=sk-... pseolint orchestrate https://example.com \\ --max-cost 3 \\ --ndjson session.ndjson \\ --manifest-out manifest.json ``` Live event stream to stdout with ANSI colors: tool calls, results, thoughts, per-step token + cost + duration, watchdog firings, budget warnings, session terminus. End-of-run summary: verdict + 4 category grades, page / template / domain patch counts, `valid/total` patches with the first 10 dropped patches surfaced. Three exit codes: 0 = completed + all patches valid · 1 = session didn't reach finish_audit · 2 = completed but some patches were rejected by validators (manifest still written). Options mirror the public API: `--ai-provider`, `--ai-model`, `--ai-key`, `--max-cost <usd>`, `--max-tool-calls <n>`, `--max-wall-seconds <n>`, `--watchdog <n>` (0 disables), `--ndjson <path>`, `--manifest-out <path>`, `--quiet`, `--no-color`.",
          "v0.5.0: Change-driven monitoring CLI surface Tracks `@pseolint/core` v0.5.0. New flags:",
          "`--mode <monitoring|fresh>`: explicit monitoring control. `monitoring` applies the pre-fetch decision matrix (default when prior state exists); `fresh` forces a full re-audit even when prior state is present.",
          "`--age-floor-days <n>`: minimum days since a URL's last fetch before monitoring forces a re-fetch regardless of other signals (default: 7).",
          "`--since` is now an alias for `--mode=monitoring` (back-compat). End-of-run summary line when monitoring is active: ``` Monitoring: 47/4012 URLs re-scraped (recheck=23, lastmod=12, age=8, new=4), 3965 carried forward. ``` When `fetched < intended`, the line shows both (\"X/Y (intended)\"), surfacing URLs the matrix wanted to refetch but downstream filters dropped (robots, byte budget, content-type, 4xx). Filesystem-source bypass now logs a one-line warning when explicit `--mode=monitoring` or `--since` is requested against a local source. See spec: `docs/superpowers/specs/2026-05-01-change-driven-monitoring-design.md`."
        ]
      },
      {
        "version": "0.4.3",
      ...
      },
      {
        "version": "0.2.3",
        "changes": [
          "fix(cli): run when the installed binary is a symlink (`npm link`, Windows global shim) The direct-run guard compared `import.meta.url` to a URL built from `process.argv[1]`, which diverges when the real file lives on another path (symlink/junction). The process exited 0 without running any command, including `--version` and audits.",
          "fix(publish): rewrite `workspace:*` to real semver in published dependencies The 0.2.1 tarballs shipped with `workspace:*` in their `dependencies` lists, which npm cannot resolve. Any `npx pseolint` or `npm install pseolint` silently failed. Republishing via `changeset publish` rewrites the workspace protocol to the real version range.",
          "Updated dependencies",
          "@pseolint/core@0.2.2",
          "@pseolint/mcp@0.2.2"
  ...
  {
    "name": "@pseolint/mcp",
    "channel": "npm · MCP server",
    "version": "0.7.5",
    "entries": [
      {
        "version": "0.7.5",
        "changes": [
          "Adds the missing `typecheck` script to `@pseolint/mcp`. `turbo run typecheck` skips a workspace that has no such script, silently and successfully, so this package had no static analysis reachable from `bun run typecheck`: a hard type error in `src/bin.ts` produced `Tasks: 1 successful` and exit 0. This lands alongside CI changes that make three existing gates actually run: the calibration verdict ratchet (its input file is gitignored, so it skipped itself on every CI run since it was written), the two renderer tests (they gate on a Chromium binary CI never installed), and the em dash check (never wired at all).",
          "CI was running a quarter of the core test suite. `@pseolint/core`'s test script passed an unquoted `tests/**/*.test.ts` to vitest; without bash globstar `**` collapses to `*`, so only the 44 files exactly two levels deep matched and the other 118 (everything under `tests/rules/<category>/`, which is most of the rule coverage) never ran under `bun run test`, the command both `ci.yml` and `release.yml` invoke. Dropping the argument lets vitest's own include do the work, matching every other package in the repo: 162 files, 1525 tests. Also hardens the MCP real-engine integration test. It pinned `PSEOLINT_MCP_JSON_CHAR_CAP` to a tuned 150000 so the airbyte_com fixture's JSON would stay under the cap, which made it a tripwire that fires whenever rules are added and the payload grows (a rule batch took the fixture to ~159k). The cap is now set far above any plausible payload, and the oversized-payload branch it bypasses, previously untested, gets its own case with a 1-char cap so neither side can drift.",
          "Punctuation-only sweep: every em dash in the repo is replaced with the punctuation its context calls for (colon for an elaboration or a \"Title: Subtitle\" heading, semicolon before an independent clause, comma for a loose afterthought, parentheses for a paired aside, hyphen inside numeric ranges). Rule message and fix strings are affected, so consumers doing exact string matching on finding text should re-check their matchers; rule IDs, severities, thresholds, and every documented URL are untouched. `scripts/no-em-dash.mjs` is the codemod that did it, kept for future use: `bun run lint:emdash:copy` is the blocking CI gate over newly added lines in source and docs, `bun run lint:emdash` reports the whole repo without blocking, `--write` applies the deterministic tiers, and `--write --fallback` force-resolves the remainder.",
          "Regenerate the OKF bundle so `/okf` and the MCP server's baked rule knowledge match the published catalog. `scripts/gen-okf.ts` derives from `MARKETING_RULES`, and that grew by 11 entries without the generator being re-run, so the statically-served bundle and `packages/mcp/src/okf-knowledge.ts` both sat at 31 rules while the catalog held 42. `llms.txt` points AI clients at `/okf/index.md` as one file per rule, so a stale bundle is an inventory claim the site cannot back. Also stops one number from drifting again: the folklore research article hard-coded \"59 rules\" in an FAQ answer and now reads `SCORED_RULE_COUNT` from the engine, the way `/methodology`, `llms.txt` and the landing-page rule ring already do.",
          "Updated dependencies [856c9f2]",
          "Updated dependencies [1aed975]",
          "Updated dependencies [856c9f2]",
          "Updated dependencies [856c9f2]",
      ...
      },
      {
        "version": "0.6.5",
        "changes": [
          "surface partial-coverage (`truncated`) audits in the MCP structured output The core engine flushes a `truncated: true` report (with `truncatedReason`) when its backpressure watchdog aborts a crawl mid-flight on a degraded origin; counts, risk, and the verdict are then lower bounds. The MCP server hand-builds the `structuredContent` payload per tool and omitted `truncated`, so an AI client saw a partial audit as if it were complete. It also omitted `schemaVersion`, leaving programmatic consumers unable to branch on the output contract.",
          "All three audit tools (`pseolint_audit_site`, `pseolint_explain_score`, `pseolint_check_page_technical`) now emit `schemaVersion` and, on a truncated run, `truncated` + `truncatedReason` in `structuredContent`. The matching optional fields were added to each tool's `outputSchema` with descriptions so clients know coverage is partial and to treat verdict/risk/pageCount as lower bounds.",
          "The human-readable text of `pseolint_audit_site` and `pseolint_explain_score` now prepends a \"⚠ Partial audit (origin degraded)\" warning line when the run was truncated, so chat-UI users see the caveat without inspecting structured data.",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
      ...
      },
      {
        "version": "0.6.4",
        "changes": [
          "Add MCP Registry support: a schema-valid `server.json` (namespace `io.github.ouranos-labs/pseolint`) plus the `mcpName` ownership field in `package.json`, so the server can be published to `registry.modelcontextprotocol.io` for client discovery.",
          "Align the MCP server with 2026 MCP guidelines. **Breaking:** all tools are now namespaced with a `pseolint_` prefix to avoid collisions with other MCP servers loaded alongside this one:",
          "`audit_site` → `pseolint_audit_site`",
          "`explain_score` → `pseolint_explain_score`",
          "`check_page_technical` → `pseolint_check_page_technical`",
          "`orchestrate_audit` → `pseolint_orchestrate_audit` Update any saved prompts or client configs that reference the old names. **Non-breaking improvements:**",
          "Every tool now declares an `outputSchema` and returns `structuredContent` (modern SDK pattern), so clients get validated structured data alongside the human-readable text. `pseolint_audit_site` embeds up to 100 severity-ordered findings, with `findingCount` for the true total and `findingsTruncated` when the array was shortened; structured `categories` are the four scored buckets only (sourced from core's `SCORED_CATEGORY_KEYS`, excluding the weight-0 `audit` diagnostics bucket).",
          "`isError` is now reserved for genuine execution failures. A site exceeding the risk threshold (or an orchestrate run that stops early) is a _successful_ call: read `passed` / `verdict` / `reason` / `completed` instead of `isError`. Previously these returned `isError: true`.",
          "Output is size-bounded without ever emitting malformed data: human-readable console/summary text is character-capped with a CLI pointer, and oversized `format:\"json\"` payloads collapse to a compact **valid-JSON** envelope (truncation marker + findings-free summary + CLI pointer) rather than being string-sliced into unparseable JSON or left unbounded.",
          "`pseolint_audit_site` and `pseolint_explain_score` now expose the `authorityScore` (bring-your-own domain authority, 0-100) and `sampleSeed` (deterministic stratified sampling) parameters that the engine already supported; previously documented in the README but not wired up.",
          "All numeric inputs are range-constrained with descriptive errors. The orchestrator budget ceilings are explicit MCP-surface safety bounds (core itself imposes none): max $50 / 500 tool calls / 900s, well above the conservative MCP defaults of $2 / 60 / 180.",
          "Added `idempotentHint` annotations: `true` for the read-only audit tools, `false` for `pseolint_orchestrate_audit` (LLM-driven, non-deterministic).",
          "Size caps are env-tunable for parity with the existing page-sample cap: `PSEOLINT_MCP_CHAR_LIMIT`, `PSEOLINT_MCP_JSON_CHAR_CAP`, `PSEOLINT_MCP_FINDINGS_CAP` (alongside `PSEOLINT_MCP_SAMPLE_CAP`).",
          "Added a test suite (56 tests, ~82% branch / 98% line on the server) covering two layers: contract tests that drive the real server through the MCP SDK client over an in-memory transport with the engine mocked (tool registration, schema-validated `structuredContent`, `isError` semantics, output size bounding incl. boundary cases, input-validation, option forwarding, env knobs), plus a true end-to-end suite that runs the real engine (`auditSource`/`formatJson`/ `formatConsole`) against a static filesystem fixture to catch core↔MCP drift.",
          "Updated dependencies",
          "@pseolint/core@0.6.4"
        ]
      },
      ...
      },
      {
        "version": "0.5.0",
        "changes": [
          "v0.5.0: AI orchestrator MCP tool + tracks `@pseolint/core` v0.5.0 New tool: **`orchestrate_audit`** exposes the AI-orchestrated auditor (added in `@pseolint/core` v0.5.0) to MCP hosts (Claude Desktop, Cursor, Windsurf, Claude Code). Unlike `audit_site` which lists rule findings, `orchestrate_audit` drives an LLM through 25 deterministic tools to produce a **fix manifest**: concrete copy-paste patches (rewritten H1s, JSON-LD blocks, robots.txt diffs, internal-link suggestions) validated against deterministic schemas before the result is returned. **Conservative defaults** for MCP-invoked sessions: $2 / 60 tool calls / 180 seconds wall (vs the CLI's $5 / 100 / 300). Can be raised by the caller if the user explicitly asks for a deeper run. **Two output modes**: `summary` (terse text suitable for chat UI with verdict + categories + top-3 patches per bucket) and `json` (full manifest + validation report + structured diff payload). **Cost transparency**: each invocation reports actual USD spend alongside the manifest summary. Patches that fail deterministic validators are dropped and surfaced separately so the LLM-host conversation stays grounded in what actually shipped vs what was proposed. Tracks `@pseolint/core` v0.5.0; also pulls in change-driven monitoring (audit_site monitoring runs are ~95% cheaper on sites with sitemap `<lastmod>`)."
        ]
      },
      {
        "version": "0.4.3",
    ...
    "entries": [
      {
        "version": "0.8.0",
        "changes": [
          "Make the action publishable to the GitHub Actions Marketplace. Marketplace lists only a root-level metadata file: \"Each repository must contain a single action metadata file (`action.yml` or `action.yaml`) at the root\", and sub-folder metadata files \"will not be automatically listed in the marketplace\". `packages/action/action.yml` is a sub-folder file, so however correct it was, Marketplace would never index it. Adds `/action.yml` as the marketplace-facing copy. Same action; only `runs.main` differs, because that path is relative to whichever root the file is published at: the repo root here, the package root for the `action-v1` orphan branch the release workflow publishes. Consumers can now use `ouranos-labs/pseolint@<tag>`, the form Marketplace lists, rather than the subdirectory-plus-branch ref. `tests/action-metadata-parity.test.ts` pins the two copies together (identical apart from `runs.main`, each pointing at the bundle relative to its own root, a Node runtime GitHub still supports, and the committed bundle present). This package previously had no tests at all, which `--passWithNoTests` hid. Also adds `branding`, and documents the release ordering in the package README: the action runs straight from the committed bundle with no build step at consumer runtime, so `dist/` must be rebuilt and committed BEFORE tagging or the tag ships a stale action."
        ]
      },
      {
        "version": "0.7.6",
      ...
      },
      {
        "version": "0.4.5",
        "changes": [
          "surface partial-coverage (`truncated`) audits in the GitHub Action gate The core engine now returns `AuditSummary.truncated` / `truncatedReason` when the backpressure watchdog aborts a crawl mid-flight, in which case the report is partial and verdict/risk/counts are lower bounds. The Action gated CI pass/fail solely on `summary.risk` and ignored `truncated`, so a partial audit could silently PASS a CI gate on incomplete coverage. The Action now makes the partial status unmissable. On a truncated run it emits a `core.warning` naming the reason and page count, sets two new outputs (`truncated`, `truncated-reason`), and adds a ⚠ blockquote to the PR comment (\"Partial coverage: crawl aborted … treat a pass as provisional\"). A partial run is not auto-failed (a partial pass is still informative), but a new optional input `fail-on-truncated` (default `false`) lets workflows opt into failing the check when coverage was partial even if risk is under threshold. `action.yml` now documents all outputs (`score`, `risk`, `verdict`, `pageCount`, `truncated`, `truncated-reason`) and the new input.",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
    ...
    "entries": [
      ...
      },
      {
        ...
        "changes": [
          ...
          "Fixed badge rendering: verdict badges now mount inside the result `<h3>` title (previously collided with the ⋮ action menu and inherited a flipped transform).",
          "The dev-only MCP bridge (`ws://localhost:4000`, for driving the extension from a terminal/LLM) is compile-stripped from production builds via `--define`; `bun run",
          "Removed a bundle leak: the service worker no longer includes the cheerio HTML parser (the server-only soft-404 probe moved to its own core module), keeping the extension truly dependency-free."
        ]
      },
      {
        "version": "0.0.3",
    ...
    "entries": [
      {
        "version": "0.8.1",
        "changes": [
          "Rank the newest audit per host on the leaderboard, not the newest passing one. The query filtered before deduplicating, and `DISTINCT ON (host)` runs over the already-filtered set. So a host whose newest audit scored at or above the risk cutoff did not drop off: Postgres returned its most recent PASSING audit instead. The site kept a score it no longer earned, and kept it until that older row expired. The comment directly above the query claimed the opposite (\"a site that degrades below the bar drops off\"), so this is the query being made to match its own documented intent rather than a change of policy. Patch, not minor, for that reason, though it is publicly visible: a degraded site now disappears from the listing instead of showing a stale number. Also adds `scripts/leaderboard-diagnose.ts`, a read-only funnel answering \"why is this public site missing?\". The listing applies eight gates plus an owner-hidden filter, so \"public\" and \"listed\" are very different populations and the gap is invisible from the page itself. It reports how many rows each gate removes, names every public host that is unlisted with the reason, and flags any host still being served a stale score."
        ]
      },
      {
        "version": "0.8.0",
        "changes": [
          "Publish `/folklore`, the list of checks pseolint deliberately does not run. Thirteen widely-repeated SEO rules that Google's own documentation contradicts (title and meta-description character limits, meta keywords, sitemap priority/changefreq, word-count minimums, the \"2 MB total site size\" misread), each with its primary source, its verdict, and the rule we run instead where a real documented failure sits nearby. Competing tools ship most of these, so refusing them with citations is a positioning asset rather than a missing feature, and it is the natural mirror of the existing blind-spots section on `/methodology`. The 13 entries live in `apps/web/src/lib/folklore.ts` and are the single source of truth: `bun run gen:folklore` regenerates `docs/folklore.md` from the same array, so the contributor doc and the public page cannot drift. The page carries FAQPage and BreadcrumbList JSON-LD, and is linked from `/methodology`, `sitemap.xml`, and `llms.txt`.",
          "Add `/rules` explainer pages for the 11 rules shipped in the folklore-vs-fact batch: `links/crawlable-anchors`, `links/generic-anchor-text`, `content/meta-description-presence`, `tech/language-mismatch`, `tech/hreflang-validity`, `tech/sitemap-hygiene`, `tech/meta-robots-conflict`, `tech/snippet-suppression`, `tech/robots-txt-limits`, `tech/html-size` and `tech/viewport-meta`. `llms.txt` advertised 59 rules while `/rules` indexed 29, so a crawler saw a claim it could not verify; the index, the dynamic route, the sitemap and the JSON-LD all derive from `MARKETING_RULES`, so the entries wire every surface at once. Each page documents the real implementation (thresholds, severities, and what the rule deliberately skips) and carries a worked example in its own domain, plus per-page authoritative citations. All 11 clear the existing dogfood contract in `marketing-rules.test.ts`, which runs pseolint's own engine over a reconstruction of every reference page: `spam/thin-content`, `content/unique-value`, `content/citation-coverage`, `content/common-phrase-reuse`, `aeo/content-modularity`, `aeo/answer-first`, `content/meta-uniqueness` and `aeo/citable-facts`.",
          "Move the publish control from the report to the site, and make it stick. Publishing was per-audit and reachable only by opening a report. Every path that creates an audit for a monitored domain hardcoded `isPublic: false`: the three dashboard actions, the kickoff crawl in `lib/monitoring.ts`, and the `monitor-domains` cron. So publishing a report worked until the next scheduled run minted a fresh private audit, and the site silently dropped off the leaderboard. The symptom read as \"the leaderboard is broken\". `monitored_domain` gains an `is_public` column (migration 0023, default false) and all five insert paths inherit it, so the choice survives re-audits. The control now lives on `/dashboard/[host]` as a \"Site visibility\" card; a report for a monitored domain links there instead of offering a toggle that only sets one row. One-off audits keep their per-report toggle, having no site page. Publishing also extends retention. `run-audit` stamps permanent expiry only for audits eligible AT COMPLETION, and monitored audits complete private, so existing rows still carried a 30/90-day expiry: without this a freshly published site would list and then vanish when that clock ran out. The card states the leaderboard consequence before the click rather than after, and shows the two further bars (at least 5 pages, risk below 40) with whether this site currently meets them, so \"public but not listed\" is explained instead of looking broken. Also fixes a mangled tooltip on the report toggle, where an em-dash rewrite had moved a parenthesis across a ternary and produced \"This report is public (anyone with the link can view it\".",
          "Remove the SEO folklore our own marketing surfaces were publishing. The checklist tools told users to keep titles under 60 characters and meta descriptions between 140 and 155, which is exactly what `docs/folklore.md` and `/research/seo-folklore-vs-google-docs` document as unsupported: Google states no length limit for either, and SERP truncation is display-side cropping rather than an indexing event. Same-origin contradictions are a real citability problem for a product whose pitch is being cited by answer engines. Rewritten to check what the primary sources actually document (presence, uniqueness, and the quality triggers behind Google's title rewrites) across `/tools/programmatic-seo-checklist` (static + interactive), `/tools/nextjs-programmatic-seo`, and five code comments that carried the same belief. Also corrects blind-spots spec section 1.5, which had listed title-length detection as a gap to close, and drops the unsupported `host:` directive from `robots.txt` (flagged by our own new `tech/robots-txt-limits` rule when auditing pseolint.dev).",
          "Regenerate the OKF bundle so `/okf` and the MCP server's baked rule knowledge match the published catalog. `scripts/gen-okf.ts` derives from `MARKETING_RULES`, and that grew by 11 entries without the generator being re-run, so the statically-served bundle and `packages/mcp/src/okf-knowledge.ts` both sat at 31 rules while the catalog held 42. `llms.txt` points AI clients at `/okf/index.md` as one file per rule, so a stale bundle is an inventory claim the site cannot back. Also stops one number from drifting again: the folklore research article hard-coded \"59 rules\" in an FAQ answer and now reads `SCORED_RULE_COUNT` from the engine, the way `/methodology`, `llms.txt` and the landing-page rule ring already do.",
          "Updated dependencies [856c9f2]",
          "Updated dependencies [1aed975]",
          "Updated dependencies [1aed975]",
      ...
      },
      {
        "version": "0.6.6",
        "changes": [
          "surface partial (`truncated`) audits in the web app The core engine flushes a `truncated: true` report when its backpressure watchdog aborts a crawl mid-flight (degraded origin), counts, risk, and the verdict are then lower bounds. The web app stored that summary to R2 and rendered it, but never surfaced the flag, so a degraded audit looked identical to a complete one.",
          "`/r/[slug]` now renders a prominent partial-coverage warning banner above the hero when the R2 summary has `truncated === true`, including `truncatedReason` when present. This reads straight off the summary blob, no DB column needed.",
          "The `audit` table gains `truncated` (boolean, default false) and `truncated_reason` (text) columns (migration `0021_loud_emma_frost`), mirrored from `AuditSummary` in the run-audit completion update so degraded audits are queryable/filterable without round-tripping R2.",
          "The per-domain workspace (`/dashboard/[host]`) shows a small \"⚠ Partial\" badge next to the latest-audit header when the run was truncated.",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
          "Updated dependencies",
      ...
      },
      {
        ...
        "changes": [
          ...
          "**Watched pages (Pro).** Pin up to 20 URLs per monitored domain; pinned URLs are force-refetched on every monitoring run regardless of diff-mode skip. New `watched_page` table (migration `0013_narrow_king_bedlam.sql`), server actions `addWatchedPage` / `removeWatchedPage` in `src/app/dashboard/domain-actions.ts` with full validation (SSRF guard, host-match against the monitored domain, www-equivalence, atomic 20-page cap, duplicate rejection). Adding a URL fires an immediate `audit/requested` with `force: { urls: [...] }`, gated by the same `DAILY_AUDIT_CAP` the public POST `/api/audits` route enforces, so the audit-on-add path can't bypass cost protection. When the daily cap is hit the watched row stays pinned and the URL audits on the next monitoring tick.",
          "**Engine force-include wiring.** `audit/requested` Inngest event payload now carries optional `force?: { urls?: string[] }`, threaded into `auditSource(...)` (consumes `@pseolint/core@0.5.3`). All four entry points pass watched URLs through: `monitor-domains.ts` cron, `lib/monitoring.ts` kickoff/re-activation, `domain-actions.ts` initial-add, and `domain-actions.ts` manual re-audit.",
          "**Cumulative coverage card.** Per-domain dashboard now surfaces total URLs audited across the full audit history (Postgres aggregate of `audits.pageCount`), with a 30-day-window sub-line. Hidden silently when a domain has no completed audit history. New `getCumulativeCoverage()` in `lib/monitoring.ts`; new component at `components/dashboard/cumulative-coverage-card.tsx`. `/limits` page copy updated to explain the cumulative-coverage framing for Pro monitoring.",
          "**Consolidated plan limits.** Daily caps (5/50), anon cap (3), Pro re-audit sample size (500), Pro monitoring sample size (200), and downgraded monitoring sample size (50) are now named constants (`DAILY_AUDIT_CAP`, `PRO_REAUDIT_SAMPLE_SIZE`, `PRO_MONITOR_SAMPLE_SIZE`, `DOWNGRADED_MONITOR_SAMPLE_SIZE`, `WATCHED_PAGES_CAP`) in `lib/audit-limits.ts`. Magic numbers eliminated from `audits/route.ts`, `monitoring.ts`, `domain-actions.ts`, `monitor-domains.ts`. The public-form 300-cap vs dashboard-re-audit 500-cap split is now documented in code.",
          "New `auditLog` events: `watched_page.added`, `watched_page.removed`, `watched_page.cap_reached`.",
          "Updated dependencies",
          "@pseolint/core@0.5.3"
        ]
      },
