...
export default async function TokensPage({ searchParams }: { searchParams: Promise<{ issued?: string }> }) {
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-6">
      <div>
        <h1 className="text-xl font-medium text-foreground">Upload tokens</h1>
      ...
      <form action={async (fd) => { "use server"; const out = await generateToken(String(fd.get("label"))); redirect(`/dashboard/settings/tokens?issued=${encodeURIComponent(out.token)}`); }} className="flex items-end gap-2">
      ...
      {tokens.length === 0 ? (
        <p className="rounded-[14px] border border-dashed border-border/60 p-6 text-center text-sm text-muted-foreground">
          No upload tokens yet. Generate one above to start uploading audit results from CI or the CLI.
        </p>
      ) : (
