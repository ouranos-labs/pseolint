...
const RULES_BASE = [
  ...
  },
  {
    ...
    metaDescription: "en_US, jp, and en-UK are ignored by Google rather than corrected. How tech/hreflang-validity checks every code value against CLDR before whole locales vanish.",
    ...
    spamBrainContext: "Invalid hreflang codes are not a spam signal and carry no policy risk; nothing in Google's spam policies touches them. They earn their place next to the spam rules through a side effect. When the annotation is discarded, near-identical regional pages lose the one mechanism that explains why they all deserve to exist. Nine locale variants of the same sofa description, six properly linked and three orphaned, look from outside like three surplus copies of a page that already ranks.\n\nCanonicalisation then does what it always does with duplicates: elects one representative URL and suppresses the rest. The retailer experiences this as a ranking problem in Japan and the United States and starts rewriting product copy, which is the wrong repair. The copy was never the issue; the declaration was thrown away before it could be read. Scoping the rule to code values only is what keeps that diagnosis clean, because reciprocity and return-link checks live in `tech/hreflang-consistency`, and merging the two would blur which failure you are looking at.\n\nThe x-default myth deserves naming here too, since it absorbs audit time that the real defect needs. Google's phrasing is a recommendation, not a requirement, and a set of nine valid codes with no x-default works. A set of nine codes where three are `en_US`, `jp`, and `en-UK` does not, and the documentation is explicit that the invalid ones are simply ignored.",
    faqs: [
      {
        "q": "Is en_US a valid hreflang value?",
        "a": "No. BCP-47 separates subtags with hyphens, so the correct value is en-US. The underscore form is a POSIX locale identifier that commonly leaks in from a backend i18n library, and Google ignores the annotation entirely rather than correcting the separator, which means the US alternate is never declared at all."
      },
      {
      ...
      },
      {
        ...
        "a": "No, and that is the core difficulty. There is no error, no crawl warning, and no URL Inspection flag; the annotation is discarded quietly. The only outward symptom is that a locale never receives the traffic it was configured for, which is why validating codes at build time is more reliable than waiting for a reporting surface to tell you."
      }
    ],
    relatedRules: [
      "language-mismatch",
