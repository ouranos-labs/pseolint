...
export const MARKETING_RULES: readonly MarketingRule[] = [
  {
    ...
    oneLiner:
      "Counts the substantive words on every page and flags any URL that falls below your configured minimum.",
    whatItDetects:
      "pseolint flags pages under 300 words against the floor Google's SpamBrain classifier has been tuned to since the March 5, 2024 scaled-content-abuse update (https://developers.google.com/search/docs/essentials/spam-policies). The rule extracts the page's main content text — after stripping nav, footer, and other chrome — splits on whitespace, and counts non-empty tokens. Any URL whose word count is below the threshold you pass to the rule (defaults differ per pSEO archetype: 200 for product comparators, 350 for guide-style hubs) is added to a `thinContentUrls` set and reported with the exact deficit. That set is then reused by other rules — most notably `spam/doorway-pattern` — so a thin page that also looks templated escalates from a single error to a critical signal stack. The check is intentionally cheap and deterministic; it does not try to evaluate quality, only volume of substantive prose.",
    whyItMatters:
      "Word count alone is a weak quality signal, which is precisely why SpamBrain treats it as one input among many. The danger is not a single thin page — it is a pattern of them. Once a meaningful share of a domain falls below the floor, Google's classifiers start treating the site as a low-effort generator: indexing slows, soft-404s start appearing in Search Console, and pages that were ranking for long-tail queries quietly lose impressions over a few weeks. The fix is rarely 'add 200 more words of waffle.' It is to ask whether the URL has any reason to exist at all.",
    failingExample:
    ...
    spamBrainContext:
      "Google's March 2024 core + spam update explicitly named 'scaled content abuse' as a spam policy violation regardless of whether the content was AI-generated, and the Search Quality Rater Guidelines have used 'thin content with little or no added value' as a Lowest-quality example since 2014. The May 2024 site-reputation policy then closed a related loophole — third-party content hosted on a high-authority domain. Both updates make pages-per-substantive-word the dominant ratio Google's quality systems care about. The `spam/thin-content` rule operationalises this by giving you a single number to act on.",
    faqs: [
      {
  ...
  },
  {
    ...
    oneLiner:
      "Fires when the same URL pair simultaneously trips near-duplicate, entity-swap, and at least one further structural signal — the highest-confidence spam pattern pseolint reports.",
    whatItDetects:
      "pseolint requires at least 3 independent signals to converge before firing this rule — mirroring the convergence logic Google's SpamBrain has used to enforce the doorway-pages policy (https://developers.google.com/search/docs/essentials/spam-policies#doorway-pages) since March 2015. The rule does not run a single check. It joins the output of two earlier rules — `spam/near-duplicate` (64-bit SimHash similarity above the configured 0.85 threshold) and the entity-swap detector (pages whose only meaningful diff is a swapped noun phrase) — then layers on additional confirmations: identical `structureSignature`, identical `<meta description>`, and whether either URL is already in the thin-content set (default 300-word floor). A pair only triggers `spam/doorway-pattern` once at least 3 of these signals agree. The finding fires at error severity (weight 25 in pseolint's scoring) and names both URLs alongside which signals stacked, so you can see at a glance whether you are looking at a near-duplicate problem (fix the content) or a template problem (fix the layout).",
    whyItMatters:
  ...
  },
  {
    ...
    oneLiner:
      "Pairwise SimHash comparison surfaces every page pair whose textual similarity exceeds your threshold, with the exact percentage shown for triage.",
    whatItDetects:
      "pseolint computes 64-bit SimHash signatures (Charikar 2002, adopted by Google's web indexing team in 2007) and flags page pairs at or above 0.85 similarity, the threshold that mirrors Google's near-duplicate canonicalisation ceiling named in the March 5, 2024 scaled-content-abuse policy. For each page, the rule computes the SimHash from the main content text using token-level shingling. It then compares every page against every other page — an O(n²) sweep that is fine for the page counts pSEO sites actually run (the 200-page hosted-audit ceiling completes in under a second). Hamming distance between two hashes is converted to a similarity score in [0,1]. Any pair scoring at or above the configured threshold (default 0.85, escalated to 0.90 for template-heavy sites) is recorded both as a finding and as a `PairMatch` consumed by `spam/doorway-pattern`. The finding includes the exact similarity percentage so you can sort the queue worst-first.",
    whyItMatters:
      "Near-duplicate pages don't just dilute ranking — they actively hurt it. When Google sees two highly similar URLs (above the SimHash 0.85 threshold pseolint uses by default, which mirrors the public deduplication ceiling), it picks one as canonical and demotes the other, but it also discounts the trust it places in the originating subfolder. The March 5, 2024 scaled-content-abuse policy (https://developers.google.com/search/docs/essentials/spam-policies) explicitly names 'paraphrasing existing content with minor changes' as a violation. A site with 40 near-duplicate pairs gets treated, structurally, as a 'content farm' regardless of intent. The pseolint rule fires at warning severity (weight 12), but each pair also counts as one of the 3 signals required for the much harsher spam/doorway-pattern rule (weight 25). The user-facing harm is real too: searchers click a result, find functionally the same page they saw two SERP positions ago, and learn the domain is low-signal.",
    failingExample:
    ...
    spamBrainContext:
      "SimHash itself was introduced in Charikar's 2002 paper and adopted by Google's web indexing team in 2007 specifically to deduplicate web crawl at scale. SpamBrain inherits that infrastructure; near-duplicate detection is one of the cheapest and oldest signals in the stack, which is why it is so reliably acted on. The 2022 helpful-content update made near-duplication a site-level signal in addition to a per-pair one, which is why a domain with many medium-similarity pairs gets demoted across pages that are individually fine. The 2024 scaled-content abuse policy explicitly includes 'paraphrasing existing content with minor changes' — which is exactly what SimHash above 0.85 detects.",
    faqs: [
      {
  ...
  },
  {
    ...
    oneLiner:
      "Identifies sentence-level text blocks that appear on 80%+ of your pages, then flags any URL whose word count is dominated by those repeated blocks.",
    whatItDetects:
  ...
  },
  {
    ...
    oneLiner:
      "Counts unique HTML structure signatures across the site and warns when too many pages share an identical skeleton.",
    whatItDetects:
      "pseolint warns when fewer than 30% of pages carry a structurally distinct HTML skeleton — the floor at which Google's SpamBrain (rebuilt in 2022 to score site-level helpfulness alongside per-page signals) starts treating a domain as a single template rather than N designed pages. Each parsed page carries a `structureSignature` — a hash of its HTML structure that ignores text content but preserves the sequence and nesting of element types. The rule counts how many distinct signatures exist across all pages and divides by the page count to produce a unique-ratio in [0,1]. If that ratio falls below `minUniqueRatio` (default 0.30 — meaning at least 30% of pages should have a structurally distinct shell), a single warning-severity finding is emitted at the site level. This is a holistic signal, not a per-page one: there is no list of 'failing' URLs because the problem is the site's design system, not any individual page.",
    whyItMatters:
      "Templated HTML is not in itself a spam signal — every modern CMS produces it. The signal is when templated HTML combines with templated content. SpamBrain reads the combination as 'one piece of low-effort programmatic output,' even if the underlying data is rich, because there is no surface variation for the classifier to latch onto. Sites with diverse structure (some pages have a comparison table, some don't; some have a video embed, some don't; some have a sticky TOC, some don't) communicate to the classifier that real per-page editorial decisions were made. Sites with one signature for every URL communicate the opposite. The fix is to introduce conditional structure, not to randomise it artificially.",
    failingExample:
    ...
    spamBrainContext:
      "Structural homogeneity has been a feature in spam classifiers since at least the 2009 'doorway page' updates, but it took on new prominence after the 2022 helpful-content update introduced site-level helpfulness scoring. Sites that look the same on every URL communicate 'mass production' to a classifier whose entire job is to find mass production. Google's December 2022 spam policy update on link spam mentioned 'sites that exist primarily to feed link signals' — those sites are almost always structurally homogeneous because they were built off a single template with no per-page editorial input. The March 2024 scaled-content policy formalised this: 'producing many pages with little unique value' is structurally measurable, and pseolint's `spam/template-diversity` is one of the simplest measurements available.",
    faqs: [
      {
